...
def main(i, SPECD_FFT_LEN, ML_FFT_STRIDE, WIN_LENGTH):
    ...
    files = glob.glob(WORKING_DIR + "data/Test set/1 hour files/*.wav",recursive=True)
    #files=glob.glob(sys.argv[1],recursive=True)

    fout = open(WORKING_DIR + f'data/results/autodetect{str(i).rjust(2, '0')}.txt', 'wt')
    ...
    for fn in files:
        ...
        fnk=fnk+1
        print(f'autodetect{str(i).rjust(2, '0')}.txt {str(fnk)} {filename_chopped}')
...
def other(i, a, b, c):
    print(f'autodetect{str(i).rjust(2, '0')}.txt')
